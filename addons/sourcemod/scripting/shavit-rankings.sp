...
public void Shavit_OnDatabaseLoaded()
{
	...
	FormatEx(sQuery, sizeof(sQuery),
		...
		"IF rtime > pwr THEN SET ppoints = IFNULL(ppoints * (pwr / NULLIF(rtime, 0)), 0); END IF; " ...
...
void FormatRecalculate(bool bUseCurrentMap, int track, int style, char[] sQuery, int sQueryLen, const char[] map = "")
{
	...
	if (bUseCurrentMap)
	{
		...
		if (track == Track_Main && gB_WorldRecordsCached)
		{
			...
			FormatEx(sQuery, sQueryLen,
				...
				"SET PT.points = IFNULL(%f * (%f / NULLIF(PT.time, 0)), 0) " ...
		...
		else
		{
			FormatEx(sQuery, sQueryLen,
				...
				"SET " ...
				" PT.points = "...
				" IFNULL((%f + (WR.time / 15.0)) " ...
				" * (WR.time / NULLIF(PT.time, 0)) " ...
				" * %f, 0)" ...
	...
	else
	{
		...
		FormatEx(sQuery, sQueryLen,
			...
			" * IFNULL((WR.time / NULLIF(PT.time, 0)), 0) " ...
			" * %f " ...
			";",
			gS_MySQLPrefix,
			gS_MySQLPrefix,
