...
void CreateGetWeightedPointsFunction()
{
	...
	FormatEx(sQuery, sizeof(sQuery),
		...
		RETURN total; \
		END;;",
					 gS_MySQLPrefix, sWeightingLimit, gCV_WeightingMultiplier.FloatValue);
...
void FormatRecalculate(bool bUseCurrentMap, int track, int style, char[] sQuery, int sQueryLen, const char[] map = "")
{
	...
	if (bUseCurrentMap)
	{
		...
		if (track == Track_Main && gB_WorldRecordsCached)
		{
			...
			FormatEx(sQuery, sQueryLen,
							 "UPDATE %splayertimes " ... "SET points = %f * (%f / time) " ... "WHERE style = %d AND track = 0 AND map = '%s';",
							 gS_MySQLPrefix,
		...
		{
			FormatEx(sQuery, sQueryLen,
							 "UPDATE %splayertimes AS PT " ... "INNER JOIN %swrs WR ON " ... "   PT.track = WR.track AND PT.style = WR.style AND PT.map = WR.map " ... "SET " ... " PT.points = " ... " IFNULL((%f + (WR.time / 15.0)) " ... " * (WR.time / NULLIF(PT.time, 0)) " ... " * %f, 0)" ... "WHERE PT.track %c 0 AND PT.style = %d AND PT.map = '%s';",
	...
	else
	{
		...
		FormatEx(sQuery, sQueryLen,
						 "UPDATE %splayertimes AS PT " ... "INNER JOIN %swrs AS WR ON " ... "  PT.track %c 0 AND PT.track = WR.track AND PT.style = %d AND PT.style = WR.style %s AND PT.map = WR.map " ... "INNER JOIN %smaptiers AS MT ON " ... "  PT.map = MT.map " ... "SET " ... " PT.points = " ... "   (((%f * %s) * 1.5) + (WR.time / 15.0)) " ... " * (WR.time / PT.time) " ... " * %f;",
						 gS_MySQLPrefix,
						 gS_MySQLPrefix,
						 (track > 0) ? '>' : '=',
						 style,
...
#if defined DEBUG
	PrintToServer("Recalculating points. (%s, %d, %d)", gS_Map, track, style);
#endif
