...
public int Native_InsideZoneGetID(Handle handler, int numParams)
{
    ...
    {
        if(gB_InsideZoneID[client][i] && gA_ZoneCache[i].iZoneType == iType && (gA_ZoneCache[i].iZoneTrack == iTrack || iTrack == -1))
        {
...
{
    return (bmin[0] <= point[0] <= bmax[0]) && (bmin[1] <= point[1] <= bmax[1]) && (bmin[2] <= point[2] <= bmax[2]);
}
    ...
    else
    {
        for(int i = 1; i <= MaxClients; i++)
        {
            if(IsClientInGame(i) && !IsFakeClient(i) && (!drawallzones || (gB_DrawAllZones[i] && CanDrawAllZones(i))))
            {
                ...
                if(GetVectorDistance(eyes, center) <= 2048.0 || (TR_TraceRayFilter(eyes, center, MASK_PLAYERSOLID, RayType_EndPoint, TraceFilter_World) && !TR_DidHit()))
                {
                    clients[count++] = i;
                }
            }
...
{
    if(other < 1 || other > MaxClients || gI_EntityZone[entity] == -1 || !gA_ZoneCache[gI_EntityZone[entity]].bZoneInitialized || IsFakeClient(other) || (gCV_EnforceTracks.BoolValue && gA_ZoneCache[gI_EntityZone[entity]].iZoneType > Zone_End && gA_ZoneCache[gI_EntityZone[entity]].iZoneTrack != Shavit_GetClientTrack(other)))
    {
        return;
    }
...
{
    if(other < 1 || other > MaxClients || gI_EntityZone[entity] == -1 || IsFakeClient(other) || (gCV_EnforceTracks.BoolValue && gA_ZoneCache[gI_EntityZone[entity]].iZoneType > Zone_End && gA_ZoneCache[gI_EntityZone[entity]].iZoneTrack != Shavit_GetClientTrack(other)))
    {
        return;
    }
